...
public final class AppCheckCoreBackoffWrapper: NSObject, AppCheckCoreBackoffWrapperProtocol,
  ...
  public func applyBackoff<T>(to operation: @escaping () -> Promise<T>,
                              errorHandler: @escaping (Error) -> AppCheckCoreBackoffType)
    -> Promise<T> {
    if !isNextOperationAllowed() {
      ...
      return Promise<T>(backoffError)
    }

    return operation().then { val in

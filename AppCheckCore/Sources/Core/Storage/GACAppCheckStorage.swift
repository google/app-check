...
@objcMembers
public class GACAppCheckStorage: NSObject, AppCheckCoreStorageProtocol {
    ...
    public func getToken() async throws -> AppCheckCoreToken? {
        return try await withCheckedThrowingContinuation { continuation in
            keychainStorage.getObjectForKey(tokenKey, objectClass: GACAppCheckStoredToken.self, accessGroup: accessGroup) { storedToken, error in
                if let error = error {
                    // Wrap keychain error
                ...
                } else if let stored = storedToken as? GACAppCheckStoredToken {

...
public let kAppCheckCoreMissingRecaptchaSDKMessage =
...
public func AppCheckCoreSetErrorToPointer(_ error: Error, _ pointer: NSErrorPointer) {
...
public class AppCheckCoreErrorUtil: NSObject {
  ...
  @objc
  public static func apiError(with httpResponse: HTTPURLResponse,
                              data: Data?) -> AppCheckCoreHTTPError {
  ...
  @objc
  public static func appAttestAttestationResponseError(withMissingField fieldName: String)
    -> Error {
  ...
  public static func missingRecaptchaSDKError() -> Error {
    return appCheckError(
      ...
      failureReason: kAppCheckCoreMissingRecaptchaSDKMessage,
      underlyingError: nil
    )
  }
  ...
  public static func appAttestGenerateKeyFailed(with error: Error) -> Error {
    let failureReason =
      "Failed to generate a new cryptographic key for use with the App Attest service (`generateKeyWithCompletionHandler:`); \(errorDescription(withDeviceCheckError: error as NSError))."
    return appCheckError(withCode: .unknown, failureReason: failureReason, underlyingError: error)
  }

  @objc
  public static func appAttestAttestKeyFailed(with error: Error, keyId: String,
                                              clientDataHash: Data) -> Error {
    let systemVersion = ProcessInfo.processInfo.operatingSystemVersionString
    let failureReason =
      "Failed to attest the validity of the generated cryptographic key (`attestKey:clientDataHash:completionHandler:`); keyId.length = \(keyId.count), clientDataHash = \(clientDataHash.base64EncodedString()), systemVersion = \(systemVersion); \(errorDescription(withDeviceCheckError: error as NSError))."
    return appCheckError(withCode: .unknown, failureReason: failureReason, underlyingError: error)
  }

  @objc
  public static func appAttestGenerateAssertionFailed(with error: Error, keyId: String,
                                                      clientDataHash: Data) -> Error {
    let systemVersion = ProcessInfo.processInfo.operatingSystemVersionString
    let failureReason =
      "Failed to create a block of data that demonstrates the legitimacy of the app instance (`generateAssertion:clientDataHash:completionHandler:`); keyId.length = \(keyId.count), clientDataHash = \(clientDataHash.base64EncodedString()), systemVersion = \(systemVersion); \(errorDescription(withDeviceCheckError: error as NSError))."
    return appCheckError(withCode: .unknown, failureReason: failureReason, underlyingError: error)
  }
  ...
  @objc
  public static func appCheckError(withCode code: AppCheckCoreErrorCode, failureReason: String?,

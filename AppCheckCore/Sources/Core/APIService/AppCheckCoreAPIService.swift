...
public extension AppCheckCoreAPIServiceProtocol {
  func sendRequest(with requestURL: URL,
    ...
    return try await withCheckedThrowingContinuation { continuation in
      Promise<AnyObject>(promise).then { response in
        if let response = response as? AppCheckCoreURLSessionDataResponse {
          continuation.resume(returning: response)
        } else {
          continuation
            .resume(throwing: AppCheckCoreErrorUtil
              .error(withFailureReason: "Invalid response type"))
        }
      }.catch { error in
  ...
  func appCheckToken(withAPIResponse response: AppCheckCoreURLSessionDataResponse) throws
    ...
    var parsedToken: AppCheckCoreToken?
    ...
    guard let parsedToken = parsedToken else {
      throw AppCheckCoreErrorUtil.error(withFailureReason: "Failed to parse token.")
    }
    return parsedToken
  }
}
...
public final class AppCheckCoreAPIService: NSObject, AppCheckCoreAPIServiceProtocol,
  ...
  public func appCheckTokenObjC(withAPIResponse response: AppCheckCoreURLSessionDataResponse)
    -> FBLPromise<AppCheckCoreToken> {
    let promise = Promise<AppCheckCoreToken> {
      try self.appCheckToken(withAPIResponse: response)
    }
    return promise.asObjCPromise()
  }
}

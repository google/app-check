...
NS_SWIFT_NAME(sendRequest(url:httpMethod:body:additionalHeaders:));

- (FBLPromise<GACAppCheckToken *> *)
appCheckTokenWithAPIResponse:(GACURLSessionDataResponse *)response
NS_SWIFT_NAME(appCheckToken(withAPIResponse:));

@end

@interface GACAppCheckAPIService : NSObject <GACAppCheckAPIServiceProtocol>
NS_SWIFT_NAME(AppCheckAPIService);

...
class AppCheckCoreBackoffWrapperTests: XCTestCase {
  ...
  func testBackoff1DayBackoffAfterFailure() async {
    ...
    do {
      _ = try await backoffWrapper.applyBackoffToOperation(
        operationProvider,
        errorHandler: errorHandler
      )
  ...
  private func assertBackoffInterval(
    isAtLeast minBackoff: TimeInterval,
    andAtMost maxBackoff: TimeInterval
  ) async {

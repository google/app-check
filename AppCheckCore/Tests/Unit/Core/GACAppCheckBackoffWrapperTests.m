...
@property(nonatomic) GACAppCheckBackoffOperationProvider operationProvider;
...
@property(nonatomic, copy) GACAppCheckBackoffErrorHandler errorHandler;
...
- (void)testBackoffFirstOperationAlwaysExecuted {
  ...
  [self setUpErrorHandlerWithBackoffType:GACAppCheckBackoffTypeNone];
...
- (void)testExponentialBackoff {
  ...
  // 1.1. Set up operation failure.
  [self setUpOperationError];
  [self setUpErrorHandlerWithBackoffType:GACAppCheckBackoffTypeExponential];
...
- (void)testDefaultAppCheckProviderErrorHandler {
  ...
  GACAppCheckHTTPError *HTTP503Error = [self httpErrorWithStatusCode:503];
  XCTAssertEqual(errorHandler(HTTP503Error), GACAppCheckBackoffTypeExponential);
  ...
  for (NSInteger statusCode = 400; statusCode < 600; statusCode++) {
    ...
    GACAppCheckHTTPError *HTTPError = [self httpErrorWithStatusCode:statusCode];
    XCTAssertEqual(errorHandler(HTTPError), GACAppCheckBackoffTypeExponential);
  }
}

#pragma mark - Helpers

- (void)setUpErrorHandlerWithBackoffType:(GACAppCheckBackoffType)backoffType {
  __auto_type __weak weakSelf = self;
  self.errorHandlerExpectation = [self expectationWithDescription:@"Error handler"];
  self.errorHandler = ^GACAppCheckBackoffType(NSError *_Nonnull error) {
...
- (GACAppCheckHTTPError *)httpErrorWithStatusCode:(NSInteger)statusCode {
  NSHTTPURLResponse *httpResponse =
      [[NSHTTPURLResponse alloc] initWithURL:[NSURL URLWithString:@"https://localhost"]
                                  statusCode:statusCode
                                 HTTPVersion:nil
                                headerFields:nil];
  GACAppCheckHTTPError *error = [[GACAppCheckHTTPError alloc] initWithHTTPResponse:httpResponse
                                                                              data:nil];
  return error;
}
...
- (void)assertBackoffIntervalIsAtLeast:(NSTimeInterval)minBackoff
  ...
  XCTAssert(FBLWaitForPromisesWithTimeout(20.0));

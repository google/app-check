...
class MockAppCheckCoreAppAttestService: NSObject, AppCheckCoreAppAttestService {
    ...
    func generateKey(completionHandler: @escaping (String?, Error?) -> Void) {
        ...
        switch result {
        case .success(let keyId): completionHandler(keyId, nil)
...
class MockAppAttestAPIService: NSObject, AppCheckCoreAppAttestAPIServiceProtocol {
    ...
    var getAppCheckCoreTokenArgs: [(artifact: Data, challenge: Data, assertion: Data, limitedUse: Bool)] = []
...
class FakeAppCheckBackoffWrapper: NSObject, AppCheckBackoffWrapperProtocol {
    ...
    func defaultAppCheckProviderErrorHandler() -> (Error) -> AppCheckBackoffType {
        return { error in return .none }
    }
    
    func applyBackoffToOperation(_ operation: @escaping () async throws -> Any, errorHandler: @escaping (Error) -> AppCheckBackoffType) async throws -> Any {
...
class AppCheckCoreAppAttestProviderTests: XCTestCase {
    ...
    func attestationRejectionHTTPError() -> AppCheckCoreHTTPError {
        let response = HTTPURLResponse(url: URL(string: "http://localhost")!, statusCode: 403, httpVersion: "HTTP/1.1", headerFields: nil)!
    ...
    func testGetToken_WhenExistingUnregisteredKey_Success() async throws {
        ...
        let attestKeyResponse = AppCheckCoreAppAttestAttestationResponse(artifact: artifactData, token: facToken)
        ...
        XCTAssertEqual(mockAppCheckCoreAppAttestService.attestKeyArgs.first?.clientDataHash, randomChallengeHash)
    ...
    func testGetToken_WhenUnregisteredKeyAndKeyAttestationError() async throws {
        ...
        let expectedError = AppCheckCoreErrorUtil.appAttestAttestKeyFailed(with: attestationError, keyId: existingKeyID, clientDataHash: randomChallengeHash)
    ...
    func testGetToken_WhenExistingKeyIsRejectedByApple_ThenAttestationIsResetAndRetriedOnce_Success() async throws {
        ...
        try await assertAttestationResetAndGetTokenRetryWhenExistingKeyIsRejectedWithAttestationError(invalidKeyError)
        
        resetMocks()
        let invalidInputError = NSError(domain: DCErrorDomain, code: DCError.invalidInput.rawValue, userInfo: nil)
        try await assertAttestationResetAndGetTokenRetryWhenExistingKeyIsRejectedWithAttestationError(invalidInputError)
    }
    ...
    func testGetToken_WhenKeyRegisteredAndGenerateAssertionError() async throws {
        ...
        let generateAssertionError = NSError(domain: "testGetToken_WhenKeyRegisteredAndGenerateAssertionError", code: 0)
        let clientDataHash = dataHashForAssertion(withArtifactData: storedArtifact)
        let expectedError = AppCheckCoreErrorUtil.appAttestGenerateAssertionFailed(with: generateAssertionError, keyId: existingKeyID, clientDataHash: clientDataHash)
        
        mockAppCheckCoreAppAttestService.generateAssertionResults.append(.failure(generateAssertionError))
    ...
    func testGetToken_WhenKeyRegisteredAndTokenExchangeRequestError() async throws {
        ...
        let tokenExchangeError = NSError(domain: "testGetToken_WhenKeyRegisteredAndTokenExchangeRequestError", code: 0)
    ...
    func testGetToken_WhenAssertionIsRejectedByApple_ThenResetToAttestationAndRetryOnceSuccess() async throws {
        let invalidKeyError = NSError(domain: DCErrorDomain, code: DCError.invalidKey.rawValue, userInfo: nil)
        try await assertAttestationResetAndGetTokenRetryWhenExistingKeyIsRejectedWithAssertionError(invalidKeyError)
        
        resetMocks()
        let invalidInputError = NSError(domain: DCErrorDomain, code: DCError.invalidInput.rawValue, userInfo: nil)
        try await assertAttestationResetAndGetTokenRetryWhenExistingKeyIsRejectedWithAssertionError(invalidInputError)
        
        resetMocks()
        let systemFailureError = NSError(domain: DCErrorDomain, code: DCError.unknownSystemFailure.rawValue, userInfo: nil)
        try await assertAttestationResetAndGetTokenRetryWhenExistingKeyIsRejectedWithAssertionError(systemFailureError)
    }
    ...
    func assertAttestationResetAndGetTokenRetryWhenExistingKeyIsRejectedWithAttestationError(_ error: NSError) async throws {
        ...
        let artifactData = "attestation artifact".data(using: .utf8)!
        let attestKeyResponse = AppCheckCoreAppAttestAttestationResponse(artifact: artifactData, token: appCheckToken)
        mockAPIService.attestKeyResults.append(.success(attestKeyResponse))
